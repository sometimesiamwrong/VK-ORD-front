...
export default defineConfig(({ command, mode }) => {
    ...
    const apiBaseUrl = mode === 'production'
        ? process.env.VITE_API_BASE || 'https://89.104.65.201:5000'  // HTTP вместо HTTPS для избежания SSL проблем
    ...
    return {
        server: {
            ...
            proxy: command === 'serve' ? {
                '/api': {
                    target: process.env.VITE_API_BASE || 'https://89.104.65.201:5000',  // HTTP вместо HTTPS
                    changeOrigin: true,
